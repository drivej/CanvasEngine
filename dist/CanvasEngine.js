...
var CanvasEngine = (() => {
  ...
  __export(main_exports, {
    ...
    CanvasElementRenderer: () => CanvasElementRenderer,
  ...
  var _addComplexStringPropTween = function _addComplexStringPropTween2(target, prop, start, end, setter, stringFilter, funcParam) {
    var pt = new PropTween(this._pt, target, prop, 0, 1, _renderComplexString, null, setter), index = 0, matchIndex = 0, result2, startNums, color, endNum, chunk, startNum, hasRandom, a;
    pt.b = start;
    pt.e = end;
    start += "";
    end += "";
  ...
  var _tweenComplexCSSString = function _tweenComplexCSSString2(target, prop, start, end) {
    ...
    var pt = new PropTween(this._pt, target.style, prop, 0, 1, _renderComplexString), index = 0, matchIndex = 0, a, result2, startValues, startNum, color, startValue, endValue, endNum, chunk, endUnit, startUnit, relative, endValues;
    pt.b = start;
    pt.e = end;
    start += "";
    end += "";
  ...
  var LoaderMax = /* @__PURE__ */ (function() {
    ...
    function load(config3) {
      if (!Array.isArray(config3.images)) {
        config3.images = [config3.images];
      }
      if (!Array.isArray(config3.onCompleteParams)) {
        config3.onCompleteParams = [];
      }
      if (typeof config3.onComplete !== "function") {
        config3.onComplete = function() {
        };
      }
      config3.images = config3.images.filter(function(e) {
        return String(e).length > 0;
      });
      for (var i = 0; i < config3.images.length; i++) {
        var img = String(config3.images[i]);
        if (config3.pattern) {
          img = config3.pattern.replace("%IMAGE%", img);
        }
        if (config3.root) {
          img = config3.root + img;
        }
        config3.images[i] = img;
        loadImage(img);
      }
      queue.unshift(config3);
      checkQueue();
    }
  ...
  var CanvasTextConverter = /* @__PURE__ */ (function(config3) {
    var SPACE_CHAR = "_-_-_-_-_-";
    var totalHeight;
    function render(text, config4, styleSheet) {
      totalHeight = 0;
      var cvs = document.createElement("canvas");
      var scale = config4 && isNumeric(config4.scale) ? config4.scale : 1;
      cvs.width = (config4 && isNumeric(config4.width) ? config4.width : 500) * scale;
    ...
    function textDataToCanvas(blocks, styles, cvs, scale, getDynamicHeight) {
      ...
      function newBlock(config4) {
        var found = currentLine.blocks.filter((n) => n.type == "text");
        currentBlock = extend({ width: 0, firstTextBlock: config4.type == "text" && found.length == 0 }, config4);
  ...
  function UIMouse(config3) {
    ...
    var _root = config3.root;
    var _targ = config3.target;
    this.config = config3;
  ...
  function CanvasElement2(config3) {
      ...
      Object.assign(
        {
          ...
          repeatX: false,
          repeatY: false,
          mouseEnabled: config3.draggable ? true : false,
          smoothing: false,
          physics: null,
          vector: false,
          html: false,
  ...
  function CanvasStage(config3) {
    ...
    this.dragFactor = 1;
    var TweenEngine = gsapWithCSS;
    var _config2 = Object.assign(
      {
        width: 100,
        height: 100,
        wheelZoom: true,
    ...
    if (config3.userBounds) {
      this.userBounds.set(config3.userBounds);
    }
    if (config3.cameraBounds) {
      this.cameraBounds.set(config3.cameraBounds);
    }
    if (config3.maxWidth) {
      this.maxWidth = config3.maxWidth;
    }
    if (config3.maxHeight) {
      this.maxHeight = config3.maxHeight;
    }
    ...
    this.mouse = _config2.mouseEnabled !== false ? new UIMouse({ target: cvs, root: this.root }) : false;
    var elementIdRef = {};
    ...
    function render() {
      ...
      ctx.fillStyle = _config2.fillStyle;
      ctx.fillRect(0, 0, cvs.width, cvs.height);
      updateTree(r);
      while (k--) {
        e = a[k];
    ...
    if (this.mouse) {
      ...
      this.root.on(UIEventTypes.WHEEL, function(e) {
        if (e.mouse.isDown && this.dragTarget !== this && this.dragTarget !== false) {
          ...
          if (_config2.wheelZoom && self.dragZ !== false) {
    ...
    this.setSize(_config2.width, _config2.height);
    updateWindowSize(true);
