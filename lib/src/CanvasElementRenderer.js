...
const CanvasBitmapRenderer_1 = require("./CanvasBitmapRenderer");
const CanvasImageLoader_1 = require("./CanvasImageLoader");
...
function CanvasElementRenderer() {
    ...
    if (this._rotation !== 0) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (p.rotation !== 0) {
                ...
                ctx.rotate(p.rotation * CanvasUtils_1.rad);
                ctx.translate(-p._tx, -p._ty);
            }
        }
    }
    ...
    if (this.rotation !== 0) {
        ...
        ctx.rotate(this.rotation * CanvasUtils_1.rad);
        ctx.translate(-this._tx, -this._ty);
    }
    if (this.clip) {
        ctx.beginPath();
    ...
    if (this.fillStyle) {
        ...
        if (this.repeatX === true) {
            ...
            if (this.borderRadius !== false) {
                ...
                (0, CanvasUtils_1.drawRoundedRect)(ctx, this.__x, this.__y, this.__width, this.__height, this.borderRadius, this.fillStyle);
            }
            else {
    ...
    if (this.bitmapData !== false) {
        CanvasBitmapRenderer_1.CanvasBitmapRenderer.call(this);
    }
    else if (this.html !== false) {
        CanvasTextRenderer_1.CanvasTextRenderer.call(this);
    }
    else if (this.image !== false) {
        CanvasImageLoader_1.CanvasImageLoader.call(this);
    }
    if (this.div !== false) {
