...
exports.UIEventTypes = exports.UIElement = void 0;
exports.UIEvent = UIEvent;
exports.UIMouse = UIMouse;
const CanvasEvent_1 = require("./CanvasEvent");
...
class UIElement {
    constructor(props) {
        ...
        function addChild(c) {
            ...
            if (c.viewport === false)
                c.viewport = self.viewport;
            self.children.push(c);
            sortChildren();
            if (self.root.addRootNode)
        ...
        function trigger(evt, bubble) {
            if (evt.className != 'UIEvent') {
                evt = new CanvasEvent_1.CanvasEvent(evt);
            }
            ...
            if (evt.type === exports.UIEventTypes.DOWN && self.draggable === true) {
                // && evt.currentTarget===self){
                self.mouseData = { down: { x: self.x, y: self.y, z: self.z } };
                if (self.root.dragTarget !== self) {
                    self.trigger(exports.UIEventTypes.DRAG_START);
...
function UIMouse(config) {
    ...
    function UIEventLookup(str) {
        var EVENTS = {
            mousedown: exports.UIEventTypes.DOWN,
            mouseup: exports.UIEventTypes.UP,
            mousemove: self.isDown ? exports.UIEventTypes.DRAG : exports.UIEventTypes.MOVE,
            mouseleave: exports.UIEventTypes.LEAVE,
            mousewheel: exports.UIEventTypes.WHEEL,
            touchstart: exports.UIEventTypes.DOWN,
            touchend: exports.UIEventTypes.UP,
            touchmove: self.isDown ? exports.UIEventTypes.DRAG : exports.UIEventTypes.MOVE,
            touchcancel: exports.UIEventTypes.LEAVE,
            pointerdown: exports.UIEventTypes.DOWN,
            pointerup: exports.UIEventTypes.UP,
            pointermove: self.isDown ? exports.UIEventTypes.DRAG : exports.UIEventTypes.MOVE,
            pointerleave: exports.UIEventTypes.LEAVE
        };
        return EVENTS[str] || '';
    }
    function handleMouse(e) {
        ...
        var EVENT = new CanvasEvent_1.CanvasEvent({ type: UIEventLookup(e.type), mouse: self, pageX: self.position.x, pageY: self.position.y });
        switch (EVENT.type) {
            case exports.UIEventTypes.WHEEL:
                ...
                break;
            case exports.UIEventTypes.DOWN:
                self.isDown = true;
                ...
                break;
            case exports.UIEventTypes.MOVE:
                break;
            case exports.UIEventTypes.DRAG:
                ...
                break;
            case exports.UIEventTypes.LEAVE:
            case exports.UIEventTypes.UP:
                if (self.isDown === true) {
                    self.isDown = false;
                }
                break;
        ...
        if (self.isDown === false) {
            var EVENT_OVER = new CanvasEvent_1.CanvasEvent({ type: exports.UIEventTypes.OVER, mouse: self, pageX: self.position.x, pageY: self.position.y });
            ...
            if (OVER_TARGET !== null && LAST_OVER_TARGET !== OVER_TARGET) {
                if (LAST_OVER_TARGET !== false) {
                    ...
                    // $(_targ).removeClass();
                    var EVENT_OUT = new CanvasEvent_1.CanvasEvent({ type: exports.UIEventTypes.OUT, mouse: self, pageX: self.position.x, pageY: self.position.y });
        ...
        hit.trigger(EVENT);
        if (hit.willTrigger({ type: exports.UIEventTypes.CLICK })) {
            // $(_targ).addClass('cursor-pointer');
            _targ.classList.add('cursor-pointer');
        }
        // click event
        // note: click can be stoppe with a stopPropagation on the up event
        if (EVENT._stopPropagation === false && EVENT.type === exports.UIEventTypes.UP && self.vector.maxLength < 3) {
            var EVENT_CLICK = new CanvasEvent_1.CanvasEvent({ type: exports.UIEventTypes.CLICK, mouse: self, pageX: self.position.x, pageY: self.position.y });
